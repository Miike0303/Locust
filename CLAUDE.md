...
- Provider line-collapse (multi-line sources): **audit closed, all 9 engines have a verdict.** Fixed for RPG Maker MV/MZ + VX Ace + **YU-RIS** via `rpgmaker_mv::rewrap_to_source_width` (fires only when the source itself is multi-line and the translation exceeds the source's widest line; single-line sources are name slots and never gain a break). YU-RIS wired on real-Injuu evidence: 52/22506 sources are author hard-wraps mid-sentence (widths 40-143), **zero** contain a backslash so `visible_len` cannot mis-count, and `escape_c_light` drops bare CR anyway so rejoining on LF reproduces the original escapes. **Not affected** (per-line records, or newlines impossible in sources): Wolf RPG, KiriKiri, QSP, TyranoBuilder, NScripter; Ren'Py also auto-wraps, so a hard re-wrap would be harmful. **Must NOT be re-wrapped — newlines are semantic, not width: Unity** (BOXMAN: 125/136 multi-line sources are Naninovel `@cmd` blobs where the newline is a statement separator; re-wrapping corrupts the script. `looks_like_naninovel_script` now filters these at extract) and **Unreal `.locres`** (Last Hope patch pak, 12769 locres entries: 39/41 multi-line break at sentence/paragraph boundaries — credits lists, placeholder-separated hint blocks — and widest lines run to 164 chars, median 86, i.e. the widget auto-wraps). Diagnostic that settled every case: extract a real game and classify whether breaks land mid-sentence (author wrapped to width, re-wrap correct) or at sentence/statement boundaries (separators or paragraphs, re-wrap destructive)
...
- RM MZ POR `.jsono` + Iavra multi-pack extract: done (prefer `en`); inject Replace re-encodes `.jsono`; inject Add writes `lang_*_{lang}.jsono` packs; inject **re-wraps pack values to the source line width** (providers return one flat line and overflow the message box — single-line sources are name slots and never gain a break). Same re-wrap on MV/MZ Replace inject and VX Ace array fields (`@description`) via `rpgmaker_mv::rewrap_to_source_width`
- RM MZ UI language registration: `locust register-lang <game> -l es --label Español` (Iavra + VisuMZ langs arrays + Map boot choices; `*.bak-locust`); **desktop/server**: `POST /api/register-lang` + Tauri `register_lang` + Inject modal “Register … in game UI” for rpgmaker-* formats (**optional menu label** when one lang selected, same as CLI `--label` (**persisted** via `locust.inject.regLabel`))
- Apply patch from URL: `locust apply <game> --url https://…/patch.zip` + desktop/server `zip_url` (desktop validates http(s) only + mutually exclusive with local path; **Patch modal**: shared `resolvePatchSource` (URL parser requires host; soft non-`.zip` path hint), disable Verify/Apply until valid source, inline URL errors, remember last path/URL; register-lang warns when no UI patterns matched; **Inject modal**: optional “after inject also register-lang” for rpgmaker-* (persisted); inject modes filtered by `supported_modes` — **Replace-only formats default to Direct** and hide Add)
