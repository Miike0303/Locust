...
- Provider line-collapse (multi-line sources): fixed for RPG Maker MV/MZ + VX Ace via `rpgmaker_mv::rewrap_to_source_width` (fires only when the source itself is multi-line and the translation exceeds the source's widest line; single-line sources are name slots and never gain a break). Audited **not affected** (per-line records, or newlines impossible in sources): Ren'Py, Wolf RPG, KiriKiri, QSP, TyranoBuilder, NScripter. **Affected, deliberately unfixed**: **YU-RIS** (real Injuu extract: 52/22506 sources are author hard-wraps mid-sentence, widths 40-143; inject re-escapes verbatim `yuris.rs:723`/`:819` — needs a plain char-width fn since `visible_len` mis-counts literal backslashes, and CRLF sources must rejoin as CRLF); **Unity** (structural TextAsset/MonoBehaviour/TextMesh/GUIText + heuristic keep newlines by design, `unity.rs:1465`); **Unreal** `.locres` (`unreal_locres.rs:472`, no length budget). Unity/Unreal blocked on an unknown: whether the widget auto-wraps at runtime, in which case a hard re-wrap is worse than the flat line
