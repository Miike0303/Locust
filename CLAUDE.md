...
- Provider line-collapse (multi-line sources): fixed for RPG Maker MV/MZ + VX Ace + **YU-RIS** via `rpgmaker_mv::rewrap_to_source_width` (fires only when the source itself is multi-line and the translation exceeds the source's widest line; single-line sources are name slots and never gain a break). YU-RIS wired to the shared helper on real-Injuu evidence: 52/22506 sources are author hard-wraps mid-sentence (widths 40-143), **zero** contain a backslash so `visible_len` cannot mis-count, and `escape_c_light` drops bare CR anyway so rejoining on LF reproduces the original escapes. **Not affected** (per-line records, or newlines impossible in sources): Wolf RPG, KiriKiri, QSP, TyranoBuilder, NScripter; Ren'Py also auto-wraps, so a hard re-wrap there would be harmful. **Unity: must NOT be re-wrapped** — measured on BOXMAN, 125/136 multi-line sources are Naninovel `@cmd` blobs where the newline is a statement separator (re-wrapping would corrupt the script; `looks_like_naninovel_script` now filters these at extract), and the rest are `<b>`-marked rich text with blank-line paragraph breaks that the widget wraps itself. **Unreal `.locres` stays undecided** (`unreal_locres.rs:472`, verbatim, no length budget) — no locres corpus on disk to classify break style; treat Unity's outcome as a reason for caution
