...
- Provider line-collapse (multi-line sources): fixed for RPG Maker MV/MZ + VX Ace + **YU-RIS** via `rpgmaker_mv::rewrap_to_source_width` (fires only when the source itself is multi-line and the translation exceeds the source's widest line; single-line sources are name slots and never gain a break). YU-RIS was safe to wire to the shared helper on evidence from the real Injuu extract: 52/22506 sources are author hard-wraps mid-sentence (widths 40-143), **zero** of them contain a backslash (only 4 in the whole game, all engine debug/paths), so `visible_len` cannot mis-count; and `escape_c_light` drops bare CR anyway, so rejoining on LF reproduces the file's original escapes. Audited **not affected** (per-line records, or newlines impossible in sources): Ren'Py (also auto-wraps, a hard re-wrap would be harmful), Wolf RPG, KiriKiri, QSP, TyranoBuilder, NScripter. **Affected, still unfixed**: **Unity** (structural TextAsset/MonoBehaviour/TextMesh/GUIText + heuristic keep newlines by design, `unity.rs:1465`) and **Unreal** `.locres` (`unreal_locres.rs:472`, no length budget) — both blocked on an unknown the format does not encode: whether the widget auto-wraps at runtime, in which case a hard re-wrap is worse than the flat line
