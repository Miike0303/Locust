...
- KiriKiri/KAG is Experimental (loose .ks + unencrypted XP3 + FE FE 0/1/2; patch.xp3 write; synthetic fixtures + **real extract E2E** Ochiru Hitozuma `patch2.xp3` ~34k + Taimanin Asagi Premium Box multi-xp3 (**~299k → ~95k → ~19.5k** after tag/`\` filters + **script-key dedupe**); **real inject E2E** temp Ochiru: `--direct` → `patch.xp3` 3/3; drops pure ellipsis/dot filler + **pure `[tag]…` lines** (KAG trailing `\` continuation + **nested `[]` in attributes** via depth scan); **multi-source dedupe** by normalized script key (`scenario/` stripped): prefer `patchN.xp3` > other XP3 > loose, tool dumps (`unencrypted/`, VNTranslationTools) lowest; no cxdec)
