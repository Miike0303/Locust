...
- Length-retry: first-pass **HARD MAX** for budgets ≤12 bytes; up to **2** length-aware retries quoting previous text + exact excess. Re-measure grok-sub en→es on Unity fixture: **0 oversize** (was 3, then 1): e.g. `New Game`→`Nueva Pt` (8), `Load Game`→`Cargar J` (8), `Options`→`Opcns` (5); `locust validate` clean. Unit tests cover 1st/2nd retry fit + still-oversize
