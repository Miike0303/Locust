...
fn test_unity_oot_extract() {
    ...
    let dialogue = entries
        .iter()
        .filter(|e| e.tags.contains(&"dialogue".to_string()))
        .count();
    let menu = entries
        .iter()
        .filter(|e| e.tags.contains(&"menu".to_string()))
        .count();
    ...
    for e in entries.iter().take(15) {
        ...
        println!(
            "  [{}] ({}) {}",
            &e.id[..e.id.len().min(30)],
            ctx,
            &e.source[..e.source.len().min(60)]
        );
    }

    assert!(
        entries.len() > 1000,
        "Should have lots of dialogue, got {}",
        entries.len()
    );
}
