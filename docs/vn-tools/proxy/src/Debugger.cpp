...
void Debugger::Log(const wchar_t* pMessage, ...)
{
    ...
    // Also append to a fixed log file for headless diagnostics.
    FILE* f = _wfopen(L"C:\\Users\\Mike\\AppData\\Local\\Temp\\krkr_dbg.log", L"a+, ccs=UTF-8");
    if (f)
    {
        fputws(message.c_str(), f);
        fputws(L"\r\n", f);
        fclose(f);
    }
}
