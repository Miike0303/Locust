...
const KIND_BADGE: Record<string, string> = {
  MissingPlaceholder: "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300",
  ExtraPlaceholder: "bg-orange-100 text-orange-800 dark:bg-orange-900/40 dark:text-orange-300",
  ExceedsCharLimit: "bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300",
  ExceedsBinarySlot: "bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300",
  EmptyTranslation: "bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200",
  IdenticalToSource: "bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300",
};

function kindDetail(kind: ValidationKind): string | null {
  if (typeof kind === "string") return null;
  if ("MissingPlaceholder" in kind) return `missing ${kind.MissingPlaceholder.placeholder}`;
  if ("ExtraPlaceholder" in kind) return `extra ${kind.ExtraPlaceholder.placeholder}`;
  ...
  return (
    <div className="mt-4">
      ...
      <div className="space-y-2 max-h-40 overflow-y-auto">
        {withMissing.map((f) => {
          const name = f.font_name || f.font_path.split(/[/\\]/).pop() || f.font_path;
          const sample = f.missing_chars.slice(0, 24).join(" ");
          const more = f.missing_count > 24 ? ` +${f.missing_count - 24} more` : "";
          return (
            <div
              key={f.font_path}
            ...
            >
              ...
              <div className="text-xs text-gray-500 mt-0.5">
                {f.missing_count} missing glyph{f.missing_count === 1 ? "" : "s"} ·{" "}
...
export default function ValidationResultsModal({
  ...
  const kindEntries = Object.entries(validation.by_kind || {}).sort((a, b) => b[1] - a[1]);
  ...
  return (
    <div className={MODAL_BACKDROP_CLASS}>
      <div ref={dialogRef} {...dialogProps} className={modalPanelClass("max-w-2xl max-h-[85vh] flex flex-col")}>
        ...
        <div className="px-5 py-4 overflow-y-auto flex-1 space-y-4">
              ...
              {kindEntries.map(([kind, n]) => (
                <span
                  ...
                  className={clsx(
                    ...
                    KIND_BADGE[kind] || "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300"
                  )}
                >
                  {kind}: {n}
                </span>
          ...
          {issues.length === 0 ? (
            <div className="py-10 text-center text-gray-500">
              <Shield size={32} className="mx-auto mb-2 text-emerald-500 opacity-80" />
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-2">Click an issue to open that string in the editor.</p>
            </div>
          )}
